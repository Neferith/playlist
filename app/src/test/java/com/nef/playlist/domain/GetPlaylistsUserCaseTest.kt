...
class GetPlaylistsUserCaseTest {
    ...
    companion object {
        ...
        private val ERROR_STATE = DataState.Error(ERROR)
    }
    ...
    @Test
    fun `network case Error`() = runTest {


        coEvery { playlistRepository.getPlaylistFromNetwork() } throws ERROR
        // When
        val result = getPlaylistsUserCase.invoke(false).last()
        io.mockk.coVerify(exactly = 0) { playlistRepository.getPlaylistFromCache() }
        io.mockk.coVerify(exactly = 1) { playlistRepository.getPlaylistFromNetwork() }
        io.mockk.coVerify(exactly = 0) { playlistRepository.insertInCache(any()) }
        assertThat(result).isEqualTo(ERROR_STATE)
    }
